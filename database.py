...
class Database:
    ...
    def add_event(self, event_data, guild_id: str):
        """添加新的 CTF 比赛"""
        conn = sqlite3.connect(self.db_file)
        c = conn.cursor()
        ...
        try:
            c.execute(
                """
                INSERT OR REPLACE INTO ctf_events 
                (event_id, guild_id, name, start_time, end_time, event_type, weight, 
                location, official_url, ctftime_url, added_time)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    event_data["event_id"],
                    ...
                    event_data["weight"],
                    event_data["location"],
                    event_data["official_url"],
                    event_data["ctftime_url"],
